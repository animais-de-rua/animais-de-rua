...
class LocalCache
{
    ...
    public static function headquarters_territories_acting(): array
    {
        return Cache::rememberForever('headquarters_territories_acting', function (): array {
            ...
            $territoriesRaw = DB::table('headquarters_territories')
                ...
                ->select(['territories.id', 'territories.name', 'territories.parent_id', 'headquarters_territories.headquarter_id']) // Make sure you prefix tables correctly
                ->groupBy(['territories.id', 'territories.name', 'territories.parent_id', 'headquarters_territories.headquarter_id']) // Include all selected columns
                ->orderByRaw('LENGTH(territories.id), territories.id')
                ->get();
    ...
    public static function processes_urgent(): Collection
    {
        return Cache::rememberForever('processes_urgent', function () {
            return Process::query()
                ->select(['id', 'name', 'address', 'specie', 'history', 'latlong', 'images', 'created_at', 'updated_at'])
                ->where('status', 'waiting_godfather')
                ->where('urgent', 1)
